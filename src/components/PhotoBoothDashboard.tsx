  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onError = () => { video.removeEventListener("canplay", onCanPlay); video.removeEventListener("error", onError); reject(new Error("Video failed to load")); };
        video.addEventListener("canplay", onCanPlay);
        video.addEventListener("error", onError);
        setTimeout(() => { video.removeEventListener("canplay", onCanPlay); video.removeEventListener("error", onError); resolve(); }, 5000);
      });
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden rounded-xl border border-border bg-surface-1 shadow-sm">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-2 overflow-hidden">
        ...
        {/* ═══════ LEFT COLUMN: Camera + Styles ═══════ */}
        <div className="flex flex-col border-r border-border overflow-hidden">

          {/* ── Camera section ── */}
          <div className="flex flex-1 flex-col items-center justify-center gap-2 border-b border-border p-3 min-h-0">
            <div className="relative w-full max-w-[280px] overflow-hidden rounded-xl border-2 border-border bg-black" style={{ aspectRatio: "1 / 1" }}>
              <video
                ref={videoRef}
              ...
              {cameraActive && !capturedImage && (
                <div style={{ position: "absolute", bottom: 10, left: 0, right: 0, display: "flex", justifyContent: "center" }}>
                  <button onClick={capturePhoto} disabled={!cameraReady}
                    className="flex h-11 w-11 items-center justify-center rounded-full border-4 border-white bg-white/20 shadow-lg backdrop-blur-sm transition-all hover:bg-white/40 hover:scale-105 active:scale-95 disabled:opacity-30"
                    title="Take photo">
                    <div className="h-7 w-7 rounded-full border-2 border-white bg-white/80" />
                  </button>
                </div>
              )}
              ...
              {!cameraActive && !capturedImage && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4 text-center">
                  ...
                  <p className="text-[11px] text-white/35">Use camera or load an image</p>
                  {cameraError && (
                    <div className="flex items-center gap-1 rounded border border-red-500/30 bg-red-500/10 px-2 py-1 text-[9px] text-red-400">
              ...
              {!cameraActive ? (
                ...
                <>
                  <button onClick={clearCapture}
                    className="flex items-center gap-1 rounded-lg border border-border bg-surface-2 px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground hover:text-foreground">
                    <RotateCcw className="h-3 w-3" /> Retake
                  </button>
                  <button onClick={stopCamera}
                    className="flex items-center gap-1 rounded-lg border border-border bg-surface-2 px-2 py-1.5 text-[11px] text-muted-foreground hover:text-foreground">
                    <CameraOff className="h-3 w-3" />
                  </button>
                </>
              ) : (
                <button onClick={stopCamera}
                  className="flex items-center gap-1 rounded-lg border border-border bg-surface-2 px-3 py-1.5 text-[11px] font-medium text-muted-foreground hover:text-foreground">
                  <CameraOff className="h-3 w-3" /> Stop
                </button>
              )}
              {/* Load image button — always available when no capture */}
              {!capturedImage && !cameraActive && null}
            </div>
          </div>
          ...
          <div className="flex flex-1 flex-col p-3 min-h-0">
                  ...
                  >
                    <div className="relative aspect-square w-full overflow-hidden bg-surface-2">
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent px-1 pb-0.5 pt-3">
                        <span className="text-[9px] font-semibold text-white drop-shadow-md leading-tight">{style.emoji} {style.label}</span>
                      </div>
                    </div>
                  </button>
                );
            ...
            {/* Load image alternative — shown when not captured */}
            {!capturedImage && !cameraActive && (
              <button onClick={() => fileInputRef.current?.click()}
                className="mt-1.5 flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-border bg-surface-2/50 px-3 py-2 text-[11px] text-muted-foreground hover:border-accent/40 hover:text-foreground">
                <Upload className="h-3.5 w-3.5" /> Or load an image from disk
              </button>
            )}
            ...
            {processing && (
              <div className="mt-1 flex items-center gap-1 text-[9px] text-muted-foreground">
                <Clock className="h-2.5 w-2.5" /> Each style takes ~15-30s
              </div>
            )}
          </div>
        </div>

        {/* ═══════ RIGHT COLUMN: Output 3×3 grid ═══════ */}
          ...
          {jobs.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 text-center p-6">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-muted-foreground/50">Take a photo or load an image, select styles, and click Generate</p>
              </div>
            </div>
          ) : (
            <div className="grid flex-1 grid-cols-3 grid-rows-3 gap-2 p-3">
              {jobs.map((job, index) => {
                  ...
                  >
                    {job.status === "success" && job.imageUrl ? (
                      <>
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent px-2 pb-1.5 pt-6">
                          <span className="text-[11px] font-bold text-white drop-shadow-lg">{preset.emoji} {preset.label}</span>
                        </div>
                      ...
                      <div className="flex h-full flex-col items-center justify-center gap-1.5 bg-surface-2/50">
                        <Loader className="h-6 w-6 animate-spin text-primary" />
                        <span className="text-[9px] font-medium text-muted-foreground">{preset.emoji} {preset.label}</span>
